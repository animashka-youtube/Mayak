document.getElementById("main-form").addEventListener("submit", function(event){
    ...
    //https:/documentSvoistva/documentElementHead/Title/Body.ActiveElement
    //URL/location/ReferrerCookieReadyState(complete/interactive/loading)
...
function proverka(){
    ...
    //https:/windowSvoistva.satus.screnx/outerwidthheight.scrollx&pagexoffset
    ...
    //https:/windowMetods.alert.confirm(okno+)prompt(vvod)scrollto.by.open.showmodaldialog
    const regName = /^[А-ЯЁA-Z][а-яёa-z]{2,20}$/;
    const regFam = /^[А-ЯЁA-Z][а-яёa-z]{2,30}$/; //igmsuydws
    const regEmail= /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;                          
    const regPass = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?\d)(?=.*?[\W_]).{8,}$/;
    ...
    } else if (regName.test(usernameValue) == false) { //match(regExp)
    ...
    if (familiaValue === "") {
        ...
        form.querySelector(".FamiliaNone").style = "display: npne";
        familia.classList.add("error");
    } else {
